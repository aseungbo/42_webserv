...
void WebServer::disconnect_client(int client_fd, Server &currServer, std::map<int, int> &clientsServerMap)
{
    ...
    std::cout << "clientServerMap serv Fd: " <<  clientsServerMap[client_fd] << std::endl;
    std::cout << "getClientMap client FD: " << currServer.getClientMap()[client_fd].getClientSocket() << std::endl;
    ...
    while (1)
    {
        ...
        for (int i = 0; i < new_events; ++i)
        {
            ...
            else if (curr_event->filter == EVFILT_READ)
            {
                ...
                if (fdManager.find(curr_event->ident) != fdManager.end())
                {
                    ...
                    if (currServer.getClientMap()[fdManager[curr_event->ident]].getCurrLocation().getLocationType() == LOCATIONTYPE_NORMAL || currServer.getClientMap()[fdManager[curr_event->ident]].getCurrLocation().getLocationType() == LOCATIONTYPE_CGI_DONE)
                    {
                        ...
                        std::cout << "fdManager size: " << fdManager.size() << std::endl;
                        std::cout << "fdManager fd: " << curr_event->ident << "client Fd: " << currServer.getClientMap()[fdManager[curr_event->ident]].getClientSocket() << std::endl;
                        ...
                        std::cout << "After readFile change_list size: " << change_list.size() << std::endl;
                ...
                {
                    std::cout << "Request Read client fd: " << curr_event->ident << "serverSoc: " << clientsServerMap[curr_event->ident] << std::endl;
                    ...
                    // int n = read(curr_event->ident, buf, 1023);
                    ...
                    else
                    {
                        ...
                        else if (currServer.getClientMap()[curr_event->ident].getClientBody().find("\r\n\r\n") != std::string::npos)
                        {
                            ...
                            {
                                std::cout << "Request Done client fd: " << curr_event->ident << "serverSoc: " << clientsServerMap[curr_event->ident] << std::endl;
                ...
                {
                    std::cout << "Find server event" << std::endl;
                    /* accept new client */
                    int clientSocket = 0;
                    ...
                    std::cout << "serverSoc: " << serverSocket << "cliSoc: " << clientSocket << std::endl;
                    std::cout << "[ Accept fdManager size: " << fdManager.size() << " ] "<< std::endl;
            ...
            else if (curr_event->filter == EVFILT_WRITE)
            {
                ...
                if (fdManager.find(curr_event->ident) != fdManager.end())
                {
                    // if (serverMap[clientsServerMap[fdManager[curr_event->ident]]].getClientMap()[fdManager[curr_event->ident]].getServerStatus() == SERVER_READY)
                    // {
                        // serverMap[clientsServerMap[fdManager[curr_event->ident]]].getClientMap()[fdManager[curr_event->ident]].setServerStatus(SERVER_ING);
                ...
                // else if (clientsServerMap.find(curr_event->ident) != clientsServerMap.end() && serverMap[clientsServerMap[curr_event->ident]].getClientMap().find(curr_event->ident) != serverMap[clientsServerMap[curr_event->ident]].getClientMap().end())
                {
                    ...
                    if (serverMap[clientsServerMap[curr_event->ident]].getClientMap()[curr_event->ident].getResponseClass().getStatusCode() != 0 )
                    {
                        ...
                        if (serverMap[clientsServerMap[curr_event->ident]].getClientMap()[curr_event->ident].getResponseClass().getBody().size() > 65535)
                        {
                            ...
                            else if (vecIdx < serverMap[clientsServerMap[curr_event->ident]].getClientMap()[curr_event->ident].getResponseClass().chunkedVec.size())
                            {
                                ...
                                // std::cout << "ident"<<curr_event->ident<< "size:"<< serverMap[clientsServerMap[curr_event->ident]].getClientMap()[curr_event->ident].getResponseClass().chunkedVec[vecIdx].size()<<std::endl;
                                // std::cout << "data"<<curr_event->data<< "udata:"<< curr_event->udata<<std::endl;
                                
                                // try{// std::cout << "data"<<(int)curr_event->data<< "udata:"<< curr_event->udata<<std::endl;}catch(std::exception e){exit(1);}
                                int n ;
                                if ((n =write(curr_event->ident, serverMap[clientsServerMap[curr_event->ident]].getClientMap()[curr_event->ident].getResponseClass().chunkedVec[vecIdx].c_str(), serverMap[clientsServerMap[curr_event->ident]].getClientMap()[curr_event->ident].getResponseClass().chunkedVec[vecIdx].size())) == -1)
                                    {
                        ...
                        {
                            std::cout << "response write client Fd: " << curr_event->ident << std::endl;
                            std::cout << "==============================" << std::endl;
                    ...
                    {
                        std::cout << "Request write client fd: " << curr_event->ident << "serverSoc: " << clientsServerMap[curr_event->ident] << std::endl;
                        ...
                        {   
                            ...
                            if (std::atoi(findIter->second.c_str()) == currSever.getClientMap()[curr_event->ident].getRequestClass().getBody().size())//바디사이즈까지 같을때
                            {
                                ...
                                std::cout << "After openFile change_list size: " << change_list.size() << std::endl;
